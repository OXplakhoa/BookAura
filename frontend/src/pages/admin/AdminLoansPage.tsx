...
export function AdminLoansPage() {
  ...
  return <section><div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between"><div><p className="eyebrow">Administration · Activity</p><h1 className="mt-3 flex items-center gap-3 font-display text-4xl font-bold"><Library className="text-primary" size={34} />Loans</h1><p className="mt-3 leading-7 text-muted">Review borrowing activity and return an active loan on behalf of a member.</p></div><div><label className="field-label" htmlFor="loan-status">Loan status</label><select id="loan-status" className="field-input min-w-48" value={active} onChange={(event) => { setActive(event.target.value); setPage(0); }}><option value="all">All loans</option><option value="active">Active only</option><option value="returned">Returned only</option></select></div></div>
    <div className="mt-8 space-y-4" aria-busy={loans.isFetching}>{loans.isError && <QueryError message={toDisplayError(loans.error).message} retry={() => loans.refetch()} />}{loans.data?.content.map((loan) => <LoanCard key={loan.id} loan={loan} active={!loan.returnedAt} showMember onReturn={!loan.returnedAt ? setSelected : undefined} />)}{loans.data?.content.length === 0 && <EmptyState title="No loans in this view" message="Change the status filter or wait for new borrowing activity." />}</div>{loans.data && <Pagination page={loans.data.page} totalPages={loans.data.totalPages} onPage={setPage} />}
    <ConfirmDialog open={Boolean(selected)} title="Return this member loan?" description={selected ? `Return “${selected.bookTitle}” for ${selected.memberName}. Inventory becomes available immediately.` : ""} confirmLabel="Confirm admin return" pending={returnMutation.isPending} onCancel={() => { setSelected(null); returnMutation.reset(); }} onConfirm={() => selected && returnMutation.mutate(selected.id)} error={returnMutation.isError ? <p className="mt-4 text-sm font-semibold text-danger" role="alert">{toDisplayError(returnMutation.error).message}</p> : undefined} />
  </section>;
}
