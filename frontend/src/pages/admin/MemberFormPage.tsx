...
export function MemberFormPage() {
  ...
  if (editing && detail.isPending) return <InlineLoading label="Loading member" />;
  if (detail.isError) return <QueryError message={toDisplayError(detail.error).message} retry={() => detail.refetch()} />;

  return <section><Link to="/admin/members" className="inline-flex min-h-11 items-center gap-2 text-sm font-bold text-primary"><ArrowLeft size={17} />Back to members</Link><p className="eyebrow mt-5">Administration · Accounts</p><h1 className="mt-3 font-display text-4xl font-bold">{editing ? "Edit member" : "Add a member"}</h1><p className="mt-3 text-muted">{editing ? "Registered email and roles require separate verified flows, so they remain read-only here." : "Create a USER account with an initial password and optional in-person email verification."}</p><div className="mt-7 max-w-3xl"><ApiErrorBanner error={serverError} /></div>
    <form onSubmit={handleSubmit(submit)} noValidate className="mt-7 max-w-3xl space-y-5 border border-line bg-surface p-6 sm:p-8"><div className="grid gap-5 sm:grid-cols-2"><MemberField id="fullName" label="Full name" error={errors.fullName?.message} input={<input id="fullName" className="field-input" {...register("fullName")} />} /><MemberField id="email" label="Email" helper={editing ? "Read-only" : undefined} error={errors.email?.message} input={<input id="email" type="email" className="field-input read-only:bg-stone-100" readOnly={editing} aria-readonly={editing} {...register("email")} />} /><MemberField id="phone" label="Phone" error={errors.phone?.message} input={<input id="phone" type="tel" className="field-input" {...register("phone")} />} /><MemberField id="dateOfBirth" label="Date of birth" error={errors.dateOfBirth?.message} input={<input id="dateOfBirth" type="date" max={yesterdayIso} className="field-input" {...register("dateOfBirth")} />} />{!editing && <MemberField id="initialPassword" label="Initial password" helper="8–72 chars" error={errors.initialPassword?.message} input={<input id="initialPassword" type="password" autoComplete="new-password" className="field-input" {...register("initialPassword")} />} />}<MemberField id="address" label="Address" error={errors.address?.message} input={<input id="address" className="field-input" {...register("address")} />} /></div>{!editing && <label className="flex min-h-11 cursor-pointer items-center gap-3 text-sm font-semibold"><input type="checkbox" className="size-5 accent-primary" {...register("emailVerified")} />Email verified after in-person identity check</label>}<label className="flex min-h-11 cursor-pointer items-center gap-3 text-sm font-semibold"><input type="checkbox" className="size-5 accent-primary" {...register("active")} />Account active</label><div className="flex justify-end gap-3 border-t border-line pt-6"><Link to="/admin/members" className="button button-ghost border border-line">Cancel</Link><button type="submit" className="button button-primary" disabled={save.isPending}>{save.isPending ? <LoaderCircle className="animate-spin" size={17} /> : <Save size={17} />}{save.isPending ? "Saving…" : "Save member"}</button></div></form>
  </section>;
}
