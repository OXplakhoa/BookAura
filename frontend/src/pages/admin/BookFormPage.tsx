...
export function BookFormPage() {
  ...
  const { register, handleSubmit, reset, setError, formState: { errors } } = useForm<BookForm>({ resolver: zodResolver(schema), mode: "onBlur", defaultValues: { publicationYear: currentYear, totalQuantity: 1, description: "", active: true, authorsText: "", categoriesText: "", title: "", isbn: "" } });
  useEffect(() => { if (detail.data) reset({ title: detail.data.title, isbn: detail.data.isbn, publicationYear: detail.data.publicationYear ?? currentYear, totalQuantity: detail.data.totalQuantity, authorsText: detail.data.authors.join(", "), categoriesText: detail.data.categories.join(", "), description: detail.data.description ?? "", active: detail.data.active }); }, [detail.data, reset]);
  const save = useMutation({
    ...
    onError: (error) => {
      ...
      Object.entries(display.fields).forEach(([field, message]) => { const mapped = field === "authors" ? "authorsText" : field === "categories" ? "categoriesText" : field; if (mapped in schema.shape) setError(mapped as keyof BookForm, { message }); });
    },
  });
  function submit(values: BookForm) {
    setServerError(null);
    const split = (value: string) => value.split(/[,|]/).map((item) => item.trim()).filter(Boolean);
    save.mutate({ title: values.title, isbn: values.isbn, publicationYear: values.publicationYear, totalQuantity: values.totalQuantity, authors: split(values.authorsText), categories: split(values.categoriesText), description: values.description || undefined, active: values.active });
  }
