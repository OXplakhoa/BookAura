...
export function BookFormPage() {
  ...
  return <section><Link to="/admin/books" className="inline-flex min-h-11 items-center gap-2 text-sm font-bold text-primary"><ArrowLeft size={17} />{t("admin.backBooks")}</Link><p className="eyebrow mt-5">{t("admin.catalog")}</p><h1 className="mt-3 font-display text-4xl font-bold">{editing ? t("admin.bookEdit") : t("admin.bookAdd")}</h1><p className="mt-3 text-muted">{t("admin.bookHelp")}</p><div className="mt-7 max-w-3xl"><ApiErrorBanner error={serverError} /></div>
    <form onSubmit={handleSubmit(submit)} noValidate className="mt-7 max-w-3xl space-y-5 border border-line bg-surface p-6 sm:p-8"><div className="grid gap-5 sm:grid-cols-2"><AdminField id="title" label={t("admin.title")}  error={errors.title?.message} input={<input id="title" className="field-input" {...register("title")} />} /><AdminField id="isbn" label="ISBN" helper={t("admin.isbnHelp")}  error={errors.isbn?.message} input={<input id="isbn" className="field-input" {...register("isbn")} />} /><AdminField id="publicationYear" label={t("catalog.filter.year")}  error={errors.publicationYear?.message} input={<input id="publicationYear" type="number" min="1450" max="2100" className="field-input" {...register("publicationYear", { valueAsNumber: true })} />} /><AdminField id="totalQuantity" label={t("admin.totalCopies")}  error={errors.totalQuantity?.message} input={<input id="totalQuantity" type="number" min="0" max="100000" className="field-input" {...register("totalQuantity", { valueAsNumber: true })} />} /><AdminField id="authorsText" label={t("admin.authors")} helper={t("admin.commaSeparated")}  error={errors.authorsText?.message} input={<input id="authorsText" className="field-input" {...register("authorsText")} />} /><AdminField id="categoriesText" label={t("admin.categories")} helper={t("admin.commaSeparated")}  error={errors.categoriesText?.message} input={<input id="categoriesText" className="field-input" {...register("categoriesText")} />} /><AdminField id="pageCount" label={t("admin.pageCount")} helper={t("admin.pageHelp")}  error={errors.pageCount?.message} input={<input id="pageCount" type="number" min="1" max="20000" className="field-input" {...register("pageCount", { valueAsNumber: true })} />} /><AdminField id="tagsText" label={t("admin.auraTags")} helper={t("admin.tagsHelp")}  error={errors.tagsText?.message} input={<input id="tagsText" className="field-input" {...register("tagsText")} />} /></div><div><label className="field-label" htmlFor="description">{t("admin.description")}</label><textarea id="description" rows={6} className="field-input py-3" aria-invalid={Boolean(errors.description)} aria-describedby={errors.description ? "description-error" : undefined} {...register("description")} /><FieldError id="description-error" message={errors.description?.message} /></div><label className="flex min-h-11 cursor-pointer items-center gap-3 text-sm font-semibold"><input type="checkbox" className="size-5 accent-primary" {...register("active")} />{t("admin.publicActive")}</label><div className="flex justify-end gap-3 border-t border-line pt-6"><Link to="/admin/books" className="button button-ghost border border-line">{t("admin.bookCancel")}</Link><button className="button button-primary" type="submit" disabled={save.isPending}>{save.isPending ? <LoaderCircle className="animate-spin" size={17} /> : <Save size={17} />}{save.isPending ? t("admin.bookSaving") : t("common.save")}</button></div></form>
  </section>;
}
