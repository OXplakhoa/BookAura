...
export function LoansPage({ mode }: { mode: "active" | "history" }) {
  ...
  const returnMutation = useMutation({
    ...
    onSuccess: async (returned) => {
      setSelected(null);
      setSuccess(`“${returned.bookTitle}” was returned successfully.`);
  ...
  return (
    <section>
      <div className="flex flex-col gap-5 sm:flex-row sm:items-end sm:justify-between">
        <div><p className="eyebrow">My library</p><h1 className="mt-3 flex items-center gap-3 font-display text-4xl font-bold">{active ? <Library className="text-primary" size={34} /> : <History className="text-primary" size={34} />}{active ? "Active loans" : "Reading history"}</h1><p className="mt-3 leading-7 text-muted">{active ? "Due dates and return controls for every book currently on your shelf." : "A permanent record of books you have returned."}</p></div>
        {loans.data && <p className="text-sm font-semibold tabular-nums text-muted"><span className="text-2xl font-bold text-ink">{loans.data.totalElements}</span> {active ? "active" : "returned"}</p>}
      </div>
      ...
      <div className="mt-8 space-y-4" aria-busy={loans.isFetching}>
        {loans.isPending && <div className="flex min-h-52 items-center justify-center gap-3 text-muted" role="status"><LoaderCircle className="animate-spin text-primary motion-reduce:animate-none" />Loading your loans…</div>}
        ...
        {loans.data?.content.length === 0 && <div><EmptyState title={active ? "Your active shelf is clear" : "No returned books yet"} message={active ? "Browse the catalog and borrow an available title to begin." : "Books appear here after you return your first loan."} /><div className="mt-5 text-center"><Link to="/catalog" className="button button-primary">Browse catalog</Link></div></div>}
      </div>
      {loans.data && <Pagination page={loans.data.page} totalPages={loans.data.totalPages} onPage={changePage} />}

      <ConfirmDialog open={Boolean(selected)} title="Return this book?" description={selected ? `Confirm that you are returning “${selected.bookTitle}”. Its copy will immediately become available to another reader.` : ""} confirmLabel="Confirm return" pending={returnMutation.isPending} onCancel={() => { setSelected(null); returnMutation.reset(); }} onConfirm={() => selected && returnMutation.mutate(selected.id)} error={returnMutation.isError ? <p className="mt-4 border-l-4 border-danger bg-red-50 p-3 text-sm text-red-950" role="alert">{toDisplayError(returnMutation.error).message}</p> : undefined} />
    </section>
  );
}
