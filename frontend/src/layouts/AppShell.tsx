...
const userNav = [
  { to: "/catalog", labelKey: "nav.browseShort", icon: BookOpen },
  { to: "/app/loans", labelKey: "nav.activeLoans", icon: Library },
  { to: "/app/history", labelKey: "nav.history", icon: History },
  { to: "/app/account", labelKey: "nav.account", icon: UserRound },
];
  ...
  return (
    <div className="min-h-dvh bg-canvas text-ink lg:grid lg:grid-cols-[264px_1fr]">
      ...
      <aside className="hidden border-r border-line bg-surface lg:sticky lg:top-0 lg:flex lg:h-dvh lg:flex-col">
        ...
        <nav className="flex-1 space-y-1 p-4" aria-label={t("nav.dashboardNavigation")}>
          {userNav.map(({ to, labelKey, icon: Icon }) => (
            <NavLink key={to} to={to} className={({ isActive }) => `side-link ${isActive ? "side-link-active" : ""}`}>
              <Icon size={19} aria-hidden="true" />{t(labelKey)}
            </NavLink>
          ))}
          {auth.isAdmin && (
            <NavLink to="/admin" className={({ isActive }) => `side-link ${isActive ? "side-link-active" : ""}`}>
              <ShieldCheck size={19} aria-hidden="true" />{t("nav.adminWorkspace")}
            </NavLink>
          )}
        </nav>
        <div className="border-t border-line p-4">
          <div className="mb-3 flex items-center justify-between gap-3 px-3"><LanguageSwitcher /><span className="sr-only">{t("language.label")}</span></div>
          ...
          <button type="button" onClick={handleSignOut} className="side-link w-full text-danger">
            <LogOut size={19} aria-hidden="true" />{t("common.signOut")}
          </button>
        </div>
      </aside>
        ...
        <header className="sticky top-0 z-20 border-b border-line bg-surface/95 lg:hidden">
          <div className="flex min-h-[68px] items-center justify-between gap-3 px-4">
            ...
            <div className="flex min-w-0 items-center gap-1">
              ...
              <button type="button" onClick={handleSignOut} className="icon-button shrink-0 text-danger" aria-label={t("common.signOut")}><LogOut size={20} /></button>
            </div>
          </div>
          <nav className="flex overflow-x-auto border-t border-line px-2" aria-label={t("nav.mobileDashboardNavigation")}>
            {userNav.map(({ to, labelKey, icon: Icon }) => (
              <NavLink key={to} to={to} className={({ isActive }) => `mobile-tab ${isActive ? "mobile-tab-active" : ""}`}>
                <Icon size={18} aria-hidden="true" />{t(labelKey)}
              </NavLink>
            ))}
            {auth.isAdmin && <NavLink to="/admin" className={({ isActive }) => `mobile-tab ${isActive ? "mobile-tab-active" : ""}`}><LayoutDashboard size={18} />{t("nav.admin")}</NavLink>}
          </nav>
        </header>
